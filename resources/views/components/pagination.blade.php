<div class="border-t border-gray-200 px-4 flex items-center justify-between sm:px-0">
    <div class="w-0 flex-1 flex">
		@if ($paginator->onFirstPage())
			<div class="-mt-px border-t-2 border-transparent pt-4 pr-1 inline-flex items-center text-sm leading-5 font-medium text-gray-200 focus:outline-none" disabled="true">
				<svg class="mr-3 h-5 w-5 text-gray-200" fill="currentColor" viewBox="0 0 20 20">
		...
		@foreach ($elements as $element)
			@foreach ($element as $page => $url)
				@if ($page == $paginator->currentPage())
					<a href="#" disabled
						class="-mt-px border-t-2 border-gray-500 pt-4 px-4 inline-flex items-center text-sm leading-5 font-medium text-gray-600 focus:outline-none focus:text-gray-800 focus:border-gray-700 transition ease-in-out duration-150">
						{{ $page }}
					</a>
				@else
					<a href="{{ $url }}"
    ...
    <div class="w-0 flex-1 flex justify-end">
		...
		@else
			<div class="-mt-px border-t-2 border-transparent pt-4 pr-1 inline-flex items-center text-sm leading-5 font-medium text-gray-200 focus:outline-none" disabled="true">
				Next
				<svg class="ml-3 h-5 w-5 text-gray-200" fill="currentColor" viewBox="0 0 20 20">
					<path fill-rule="evenodd" d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z" clip-rule="evenodd" />
				</svg>
			</div>
		@endif
